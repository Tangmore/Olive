            ...
            <div class="bg-white">
                ...
                <div id="form-register" class="row pt-0 pb-0 pl-4 pr-4 m-0 my-form" :class="status=='register'?'':'d-none'">
                    ...
                    <input type="password" name="upwd" id="upwd-reg" class="form-control p-2 mb-2" :class="registerForm.upwdVerifyStyle" placeholder="密码"
                        v-model="registerForm.upwd" @blur="verifyUpwd">
                    <input type="password" id="upwd-again" class="form-control p-2 mb-2" :class="registerForm.againUpwdVerifyStyle" placeholder="确认密码"
                        v-model="registerForm.againUpwd" @blur="verifyAgainUpwd">
                    <p class="w-100 text-center m-0">
                        <small>密码为6~14位数字、字母或下划线！</small>
                    </p>
                    <button id="register" type="botton" class="btn btn-success w-100 p-2 mt-2" @click="register" :disabled="!registerForm.checked">
                        <span class="h5">立即注册</span>
                    </button>
                    <p class="d-flex justify-content-center align-items-center m-auto p-2">
                        ...
                        <a>
                            <small class="text-info">《Olive用户协议》</small>
                        </a>
                    </p>
                </div>
                ...
                <div id="form-login" class="row pt-0 pb-4 pl-4 pr-4 m-0 my-form" :class="status=='login'?'':'d-none'">
                    ...
                    <button id="login" class="btn btn-success w-100 p-2 mt-2" @click="login">
                        <span class="h5">登&nbsp;录</span>
                    </button>
                </div>
                <!-- 登录表单end -->
            ...
            <div class="d-flex justify-content-center pb-4 pt-2">
                <p class="m-auto pt-1 pb-1 pl-3 pr-3">
                    ...
                    <a href="javascript:;" @click="changeStatus">
                        <small v-if='status==="login"'>马上注册</small>
                        <small v-else>马上登录</small>
                    </a>
                </p>
            </div>
        ...
        methods: {
            ...
            verifyUser() {
                ...
                if (!this.registerForm.user.trim()) {
                    this.registerForm.alert.msg.user = "请输入手机号进行注册！"
                    this.registerForm.alert.show = true;
                    this.registerForm.userVerifyStyle = "border-danger";
                    return;
                }
                if (!phoneReg.test(this.registerForm.user)) {
                    this.registerForm.alert.msg.user = "手机号格式错误！"
                    this.registerForm.alert.show = true;
                    this.registerForm.userVerifyStyle = "border-danger";
                    return;
                }
            ...
            register() {
                ...
                if (this.registerForm.userVerifyStyle == "border-success" &&
                    this.registerForm.upwdVerifyStyle == "border-success" &&
                    this.registerForm.againUpwdVerifyStyle == "border-success" &&
            ...
            login() {
                ...
                if (!this.loginForm.user.trim()) {
                    ...
                    this.loginForm.alert.msg = "请输入手机号进行登录！";
                    return;
                }
                if (!phoneReg.test(this.loginForm.user)) {
                    this.loginForm.alert.show = true;
                    this.loginForm.alert.style = "text-danger";
                    this.loginForm.alert.msg = "手机号格式错误！";
                    return;
                }
